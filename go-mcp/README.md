...
[MCP](https://modelcontextprotocol.io) server exposing the Dog SDK as
two agent tools — `dog_list` and `dog_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dog-mcp)
make build

# 2. Provide credentials via the environment
export DOG_APIKEY=sk_live_xxx
...
```jsonc
// dog_list: first page of records
{ "entity": "breed" }
{ "entity": "breed", "query": { } }

// dog_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dog-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dog \
     -- "$PWD"/dist/*/dog-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dog_list` and `dog_load` tools now appear
   in new sessions. Ask the agent to *"list breed using dog"*
   and it calls `dog_list` with `{"entity":"breed"}`.
...
```sh
export DOG_APIKEY=sk_live_xxx            # API key
export DOG_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `dog_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dog_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DOG_APIKEY` | API key sent with every request. |
| `DOG_BASE` | Optional override of the API base URL. |
