...
(0, node_test_1.describe)('ImageDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-image', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-image', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["breed01", "count01"]))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'breed/{breed_id}/{sub_breed}/images',
                method: 'GET',
                params: {
                    breed_id: setup.idmap['breed01'],
                    sub_breed: setup.idmap['sub_breed01'],
                },
            });
            ...
            const candidateId = listArr[0]?.breed_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.breed_id = candidateId;
            params.breed_id = setup.idmap['breed01'];
            params.count = setup.idmap['count01'];
        }
        else {
            params.breed_id = 'direct01';
            params.count = 'direct02';
        }
        const result = await client.direct({
            path: 'breed/{breed_id}/images/random/{count}',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-image', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-image', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["breed01", "sub_breed01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.breed_id = setup.idmap['breed01'];
            params.sub_breed = setup.idmap['sub_breed01'];
        }
        else {
            params.breed_id = 'direct01';
            params.sub_breed = 'direct02';
        }
        const result = await client.direct({
            path: 'breed/{breed_id}/{sub_breed}/images',
            method: 'GET',
...
function liveScenariosActive() { return false && process.env.DOG_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.DogSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['DOG_TEST_IMAGE_ENTID'];
