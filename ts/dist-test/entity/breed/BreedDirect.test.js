...
(0, node_test_1.describe)('BreedDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-breed', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-breed', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-breed', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-breed', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["breed01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = setup.idmap['breed01'];
        }
        else {
            params.id = 'direct01';
        }
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['DOG_TEST_BREED_ENTID'];
