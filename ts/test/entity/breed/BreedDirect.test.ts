...
describe('BreedDirect', async () => {
  ...
  test('direct-load-breed', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-breed', setup.live)) return
  ...
  test('direct-list-breed', async (t: any) => {
    ...
    if (skipIfMissingIds(t, setup, ["breed01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DOG_TEST_BREED_ENTID']
