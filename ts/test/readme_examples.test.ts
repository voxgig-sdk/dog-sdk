...
//     (\`new DogSDK(...)\` / \`DogSDK.test(...)\` become
//     \`DogSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"breed":{"test01":{"id":"test01"}},"image":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DogSDK(...)\` and any
// \`DogSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DogSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DogSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
