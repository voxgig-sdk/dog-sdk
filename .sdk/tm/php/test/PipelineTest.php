...
{
    private static function utility(): DogUtility
    {
        return new DogUtility();
    }
    ...
    private static function ctx(array $over = []): DogContext
    {
        ...
        $ctx->op = new DogOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DogError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DogOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DogMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DogOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DogMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DogOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DogMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DogSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DogMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new DogResult([]),
        ]);
        [, $err] = DogMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DogSpec(['step' => 's']),
            'response' => new DogResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DogResult([]),
        ]);
        [, $err] = DogMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DogSpec(['step' => 's']),
            'response' => new DogResponse(self::resp(200, ['v' => 1])),
            'result' => new DogResult([]),
        ]);
        [, $err] = DogMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DogSpec(['step' => 's']),
            'response' => new DogResponse(self::resp(200, ['v' => 2])),
            'result' => new DogResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DogMakeResponse::call($ctx);
    ...
    {
        $preset = new DogResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DogSpec([]),
            'response' => new DogResponse([]),
            'result' => new DogResult([]),
        ...
        [$got, $err] = DogMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DogResult([])]);
        [, $err] = DogMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DogSpec([]), 'result' => null]);
        [, $err] = DogMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DogSpec(['step' => 's']),
            'result' => new DogResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DogOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DogMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DogSpec(['step' => 's']),
            'result' => new DogResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DogOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DogMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DogResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DogSpec([]),
            'result' => new DogResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DogMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new DogError('boom', 'boom');
        $utility->fetcher = function (DogContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DogSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DogMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DogContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DogMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DogResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DogContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DogSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DogMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DogResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DogSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DogMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DogMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DogResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DogDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DogResult(['ok' => false])]);
        $this->expectException(DogError::class);
        DogDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DogResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DogMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DogBaseFeature();
        $b = new DogBaseFeature();
        DogFeatureAdd::call($ctx, $a);
        DogFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DogBaseFeature
    {
        $f = new DogBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): DogContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DogSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new DogResponse(['headers' => null]),
            'result' => new DogResult([]),
        ]);
        DogResultHeaders::call($ctx);
