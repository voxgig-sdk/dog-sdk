...
class FtCtrl extends DogControl
{
...
class FtClient
{
    ...
    private DogUtility $utility;

    public function __construct(DogUtility $utility, array $options)
    {
    ...
    public function get_utility(): DogUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DogContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DogContext $_ctx, string $_url, array $fetchdef): array {
    ...
    private static function buildUrl(DogSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DogError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DogResponse($fetched);
        }
    ...
    private function fail(DogContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DogContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
