...
class BreedEntity
{
    ...
    {
        if ($args) {
            $this->_data = DogHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = DogHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     * Load a single Breed.
     *
     * @param BreedLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed BreedLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Breed|array The loaded Breed as an assoc-array at the
     *   SDK boundary; throws DogError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DogHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws DogError on failure (item-5 convention).
     */
