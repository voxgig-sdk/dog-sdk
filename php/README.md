...
The PHP SDK for the Dog API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Breed(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->Dog(null)->load(
    ["id" => "test01"], null
);
