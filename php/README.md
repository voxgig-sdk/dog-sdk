...
The PHP SDK for the Dog API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Breed()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $breed));
```
...
```php
$client = DogSDK::test($testopts, $sdkopts);
```
...
The main class (`dog_sdk.php`) exports the SDK class
