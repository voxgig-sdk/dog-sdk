...
class DogNetsimFeature extends DogBaseFeature
{
    ...
    private function _simulate(DogContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(DogContext $ctx, array $applied): void
    {
        $client = $this->client;
