...
//    only - a typed DogContext there would make every ctx
...
final class DogOmni
{
    ...
    public static function err_from_map($m): ?DogError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DogError(is_string($code) ? $code : '', $msg);
    }
