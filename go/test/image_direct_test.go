...
func TestImageDirect(t *testing.T) {
	t.Run("direct-list-image", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"breed01", "sub_breed01"} {
	...
	t.Run("direct-load-image", func(t *testing.T) {
		...
		if setup.live {
			...
			listParams["breed_id"] = setup.idmap["breed01"]
			listParams["sub_breed"] = setup.idmap["sub_breed01"]
...
func imageDirectSetup(mockres any) *imageDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"DOG_TEST_IMAGE_ENTID": map[string]any{},
		"DOG_TEST_LIVE":    "FALSE",
		"DOG_APIKEY":       "NONE",
	})
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DOG_TEST_IMAGE_ENTID"]; ok {
