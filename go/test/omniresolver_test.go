...
import (
	...
	omni "github.com/voxgig-sdk/dog-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DogError); is {
				out := map[string]any{
					"name":    "DogError",
...
func omniCtx(arg any, client *sdk.DogSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DogSDK); is {
			client = live
