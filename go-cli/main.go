// Command Dog-cli is a CLI + REPL for the Dog SDK,
...
const entitiesHelp = "breed image"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DOG_APIKEY (api key), DOG_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DOG_APIKEY carries the API key and
	// DOG_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DOG_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DOG_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDogSDK(opts)
