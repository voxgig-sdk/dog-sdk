...
boru-driven command-line client **and** interactive REPL for the Dog
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dog-cli)
make build
...
./dog-cli list breed
./dog-cli load 1 breed            # {id:1} shorthand
./dog-cli load '{id:1}' breed       # explicit match map
./dog-cli list image

# 5. Override the API base URL for a single call
DOG_BASE=https://api.example.com ./dog-cli list breed
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dog-cli
   ```
...
```sh
./dog-cli load 1 breed          # scalar shorthand for {id:1}
./dog-cli load '{id:1}' breed     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `dog>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dog-cli`. |
...
(`./dog-cli <expr>`) and interactively (the REPL), and expressions compose the
