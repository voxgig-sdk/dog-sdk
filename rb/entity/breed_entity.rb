...
class BreedEntity
  ...
  def data_set(args)
    if args
      @_data = DogHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DogHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Breed.
  #
  # @param reqmatch [BreedLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Breed.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Breed, Hash] the loaded Breed; raises DogError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DogHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Breed>, Array] the matching Breed items; raises DogError on failure
