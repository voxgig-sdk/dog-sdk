...
class BreedEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = DogSDK.test(seed, nil)
    seen = base.Breed(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = DogConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = DogSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DOG_TEST_BREED_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    breed_ref01_data_dt0_loaded = breed_ref01_ent.load(breed_ref01_match_dt0, nil)
    breed_ref01_data_dt0_load_result = Helpers.to_map(breed_ref01_data_dt0_loaded.respond_to?(:data_get) ? breed_ref01_data_dt0_loaded.data_get : breed_ref01_data_dt0_loaded)
    assert !breed_ref01_data_dt0_load_result.nil?
    assert_equal breed_ref01_data_dt0_load_result["id"], breed_ref01_data["id"]
...
def breed_basic_setup(extra)
  ...
  client = DogSDK.test(options, extra)
  ...
  entid_env_raw = ENV["DOG_TEST_BREED_ENTID"]
  ...
  if env["DOG_TEST_LIVE"] == "TRUE"
    ...
    client = DogSDK.new(Helpers.to_map(merged_opts))
  end
