...
module DogOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if DogOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def keys
      out = view_names.map { |n| DogOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || DogOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = DogSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = DogError.new('', err_map['message'])
      end
    end
