...
describe("BreedDirect", function()
  it("should direct-list-breed", function()
    ...
    if setup.live then
      for _, _live_key in ipairs({"breed01"}) do
  ...
  it("should direct-load-breed", function()
    local setup = breed_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-breed", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function breed_direct_setup(mockres)
  ...
  local live = env["DOG_TEST_LIVE"] == "TRUE"
